function dataFromForm() {
	...
	const learningObjectives = document.getElementsByClassName("learningObjectivesColumn-1McrU");
	const taxonomies = document.getElementsByClassName("taxonomiesColumn-1Qy6B");
	if (numbers.length > 0 && titles.length > 0 && ids.length > 0 && types.length > 0 && scores.length > 0 && learningObjectives.length > 0 && taxonomies.length > 0) {
		const aggregated = [
			['Number', 'Question', 'ID', 'Type', "Score", "Taxonomy", "Objective"]
		];
		for (var i = 0; i < titles.length; i++) {
			const row = [numbers[i].innerText, titles[i].innerText, ids[i].innerText, types[i].innerText, scores[i].innerText, learningObjectives[i].innerText, taxonomies[i].innerText];
			aggregated.push(row);
		}
